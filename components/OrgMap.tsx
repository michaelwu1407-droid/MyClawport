...
function buildTeamLayout(
  ...
  // Stacking logic: 2 rows
  let rowHeights = [0, 0];
  let columnWidths = [];
  
  // Precalculate dimensions
  const colDimensions = columns.map((col, ci) => {
    ...
    return { positions, minX, minY, groupW, groupH }
  });

  // Calculate grid max dimensions
  for (let ci = 0; ci < colDimensions.length; ci += 2) {
    const w1 = colDimensions[ci].groupW;
    const w2 = ci + 1 < colDimensions.length ? colDimensions[ci + 1].groupW : 0;
    columnWidths.push(Math.max(w1, w2));
    rowHeights[0] = Math.max(rowHeights[0], colDimensions[ci].groupH);
    if (ci + 1 < colDimensions.length) {
      rowHeights[1] = Math.max(rowHeights[1], colDimensions[ci + 1].groupH);
    }
  }

  for (let ci = 0; ci < columns.length; ci++) {
    const col = columns[ci]
    const dim = colDimensions[ci]
    const isEven = ci % 2 === 0
    const pairIndex = Math.floor(ci / 2)
    
    // Calculate X position
    let xPos = 0;
    for (let j = 0; j < pairIndex; j++) {
      xPos += columnWidths[j] + COL_GAP;
    }
    
    // Calculate Y position
    const yPos = isEven ? COLUMNS_TOP : COLUMNS_TOP + rowHeights[0] + COL_GAP;
    ...
    for (const id of col.agentIds) {
      ...
      const pos = dim.positions.get(id)
      if (!agent || !pos) continue
      childNodes.push({
        id,
        type: "agentNode",
        data: agent as unknown as Record<string, unknown>,
        position: { x: pos.x - dim.minX + GROUP_PAD_X, y: pos.y - dim.minY + GROUP_PAD_TOP },
        parentId: groupId,
    ...
    columnResults.push({ groupNode, childNodes, width: dim.groupW })
    if (isEven && ci === columns.length - 1) {
      cursorX = xPos + dim.groupW + COL_GAP
    } else if (!isEven) {
      cursorX = xPos + columnWidths[pairIndex] + COL_GAP
    }
  }
